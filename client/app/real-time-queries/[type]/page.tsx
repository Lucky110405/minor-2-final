...
import { Loader2, TrendingUp, Newspaper, HelpCircle, ArrowLeft } from 'lucide-react';
...
export default function DynamicQueryPage() {
  ...
  const queryType = params.type as string;
  ...
  const renderContent = () => {
    switch (queryType) {
      ...
      default:
        return (
          <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-6`}>
            <h2 className={`text-xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Invalid Query Type</h2>
            <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'} mt-2`}>
              Please select a valid query type from the navigation menu.
            </p>
          </div>
        );
    }
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'} py-12 px-4 sm:px-6 lg:px-8`}>
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <Link
            href="/real-time-queries"
            className={`inline-flex items-center ${isDarkMode ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-gray-900'}`}
          >
        ...
        <div className="text-center mb-12">
          <h1 className={`text-4xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} mb-4`}>
            {queryType === 'stock' && 'Stock Price Tracker'}
            {queryType === 'news' && 'Market News'}
            {queryType === 'faq' && 'Financial FAQs'}
            {!['stock', 'news', 'faq'].includes(queryType) && 'Invalid Query Type'}
          </h1>
          <p className={`text-lg ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            {queryType === 'stock' && 'Get real-time stock prices and market data'}
            {queryType === 'news' && 'Stay updated with the latest market news and trends'}
            {queryType === 'faq' && 'Get answers to your financial questions'}
            {!['stock', 'news', 'faq'].includes(queryType) && 'Please select a valid query type'}
          </p>
        </div>
